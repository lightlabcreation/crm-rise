...
const getAll = async (req, res) => {
  try {
    const { category, lead_id, project_id, client_id, folder_id } = req.query;
    ...
    // Filter by folder (Removed as per user request/DB schema mismatch)
    /*
    if (folder_id === 'null') {
    ...
    */
    ...
    try {
      ...
      console.error('SQL Error in getAll:', sqlError);
...
const create = async (req, res) => {
  try {
    const { title, name, category, description, company_id, user_id, lead_id, project_id, client_id, folder_id } = req.body;
    const file = req.file;
    ...
    res.status(500).json({
      success: false,
      error: error.sqlMessage || error.message || 'Failed to upload document'
    });
  }
};
