...
const create = async (req, res) => {
  try {
    ...
    // Resolve client_id if client_id matches user_id (common frontend error) or is missing
    ...
    if (userId && (!finalClientId || parseInt(finalClientId) === userId)) {
      console.log(`Checking client profile for user_id: ${userId} (provided client_id: ${finalClientId})`);
      try {
        const [clients] = await pool.execute(
        ...
        if (clients.length > 0) {
          ...
          console.log(`Resolved correct client_id: ${finalClientId}`);
        } else {
          console.warn(`No client profile found for user_id: ${userId}`);
        }
      } catch (err) {
        console.error('Error resolving client_id:', err);
      }
    }
...
const update = async (req, res) => {
  try {
    ...
    // Use company_id from req or default to 1 (since routes are public/unauthenticated)
