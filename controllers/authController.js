...
const login = async (req, res) => {
  try {
    ...
    if (!role) {
      ...
      return res.status(400).json({
        success: false,
        error: 'Role is required (SUPERADMIN, ADMIN, EMPLOYEE, CLIENT, or FINANCE)'
      });
    }
    ...
    if (user.role === 'CLIENT') {
      try {
        const [clientData] = await pool.execute(
          `SELECT id FROM clients WHERE owner_id = ? AND is_deleted = 0`,
          [user.id]
        );
        if (clientData.length > 0) {
          client_id = clientData[0].id;
          console.log('Client ID fetched:', client_id, 'for user:', user.id);
        }
      } catch (err) {
  ...
  try {
    ...
    const [users] = await pool.execute(
      `SELECT u.id, u.company_id, u.name, u.email, u.role, u.status, u.avatar, u.phone, u.address,
       ...
       LEFT JOIN clients cl ON u.id = cl.owner_id AND cl.is_deleted = 0
